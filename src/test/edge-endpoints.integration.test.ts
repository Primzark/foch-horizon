...
const describeEdge = runEdgeIntegration && citiesUrl && propertiesUrl ? describe : describe.skip;
...
describeEdge("Supabase Edge endpoint contracts", () => {
  ...
  it("returns a property detail payload", async () => {
    ...
    const detail = await getJson<{ id: number; title: string }>(`${propertiesUrl}/${propertyId}`);
